...
function CompleteProfileContent() {
  ...
  useEffect(() => {
    setIsClient(true);
    console.log("CompleteProfilePage: Client-side rendering enabled");
  }, []);
  ...
  useEffect(() => {
    ...
    console.log("CompleteProfilePage: Auth state", {
      isAuthenticated,
      isLoading,
      user,
    ...
    if (isLoading) {
      console.log("CompleteProfilePage: Auth still loading");
      return;
    }

    if (!isAuthenticated) {
      console.log("CompleteProfilePage: Not authenticated, redirecting to /login");
      router.push("/login");
      return;
    }

    if (user?.profileCompleted) {
      console.log("CompleteProfilePage: Profile completed, redirecting to /dashboard");
  ...
  // Error state
  if (!user.role) {
    console.error("CompleteProfilePage: User role is undefined");
    return (
      <motion.div
      ...
      >
        <FiAlertCircle className="h-12 w-12 text-red-500 mx-auto" aria-hidden="true" />
      ...
      <div className="bg-white shadow-2xl rounded-2xl overflow-hidden">
        <div className="px-8 py-10 text-center">
          ...
          <h1 className="text-3xl font-bold text-gray-900">
            Complete Your {user.role === "donor" ? "Donor" : "Organization"} Profile
          </h1>
        ...
        <div className="px-8 pb-8">
          {user.role === "donor" ? <DonorProfileForm /> : <OrganizationProfileForm />}
        </div>
      </div>
    </motion.div>
  );
