...
const CampaignsPage = () => {
	...
	const [viewType, setViewType] = useState<"campaigns" | "causes">("campaigns");
	...
	// Fetch campaigns
	const { data, refetch } = useGetCampaignsQuery({
	...
	if (data) {
		...
		if (data.data) {
			...
			if (Array.isArray(responseData)) {
				...
				campaigns = responseData.campaigns.filter(
					(item) => item && Object.keys(item).length > 0
				);
			}
		}
		...
		// Try to extract from another structure
			...
			if (possibleArrays.length > 0) {
				...
				const campaignsArray =
					possibleArrays.find((item) => item.key === "campaigns") ||
					possibleArrays[0];
				if (campaignsArray) {
					campaigns = (campaignsArray.value as any[]).filter(
						(item) => item && Object.keys(item).length > 0
					);
				}
			}
	...
	const handleCreateCause = () => router.push("/dashboard/causes/create");
	...
	const handleDeleteConfirm = async () => {
		if (selectedCampaignId) {
			try {
				setDeleteError(null);
				console.log(
					"Attempting to delete campaign with ID:",
					selectedCampaignId
				);
		...
		<Box sx={{ backgroundColor: "#f5f7fa", minHeight: "100vh" }}>
			...
			>
				<Typography variant="h5" fontWeight="bold" gutterBottom>
					Your {viewType === "campaigns" ? "Campaigns" : "Causes"}
				</Typography>
				<Typography variant="subtitle2">
					Manage all your{" "}
					{viewType === "campaigns" ? "fundraising campaigns" : "causes"}
				</Typography>
			</Paper>
			...
			<Box sx={{ p: 2 }}>
							...
							<Button
								...
								onClick={
									viewType === "campaigns"
										...
										: () => setCreateCauseDialogOpen(true)
								}
								size="small"
							>
								Create {viewType === "campaigns" ? "Campaign" : "Cause"}
							</Button>
						</Stack>
					</Stack>
				</Paper>
				...
				<Box sx={{ display: "flex", justifyContent: "center", mb: 3 }}>
					...
					>
						<Button
							variant="contained"
							color={viewType === "campaigns" ? "primary" : "inherit"}
							sx={{
								px: 4,
								borderRadius: 0,
								bgcolor:
									viewType === "campaigns"
										? "primary.main"
										: "background.paper",
								color: viewType === "campaigns" ? "white" : "text.primary",
								"&:hover": {
									bgcolor:
										viewType === "campaigns" ? "primary.dark" : "grey.200",
								},
							}}
						...
						<Button
							variant="contained"
							color={viewType === "causes" ? "primary" : "inherit"}
							sx={{
								px: 4,
								borderRadius: 0,
								bgcolor:
									viewType === "causes" ? "primary.main" : "background.paper",
								color: viewType === "causes" ? "white" : "text.primary",
								"&:hover": {
									bgcolor: viewType === "causes" ? "primary.dark" : "grey.200",
								},
							}}
							onClick={() => setViewType("causes")}
						>
							Causes
						</Button>
					</Paper>
				...
				{isLoading ? (
					...
					<Alert severity="error">
						Failed to load campaigns: {JSON.stringify(error)}
					</Alert>
				) : viewType === "campaigns" ? (
						...
						>
							{filteredCampaigns.map((campaign: Campaign, index: number) => {
								try {
													...
													<Button
														...
														onClick={() =>
															campaign && campaign.id
																...
																: console.error("Missing campaign ID for view")
														}
													...
													<Button
														...
														onClick={() =>
															campaign && campaign.id
																...
																: console.error("Missing campaign ID for edit")
														}
														sx={{ fontSize: "0.8rem", minWidth: "auto" }}
													>
														Edit
													...
													<Button
														...
														onClick={() =>
															campaign && campaign.id
																...
																: console.error(
																		"Missing campaign ID for delete"
																  )
														}
														disabled={isDeleting}
														sx={{ fontSize: "0.8rem", minWidth: "auto" }}
									...
									console.error(
										`Error rendering campaign ${index}:`,
										err,
										campaign
									);
									return (
					...
					<Paper sx={{ p: 2, textAlign: "center", borderRadius: 2 }}>
						...
						{/* create cause deatail  */}
						{/* <div>
							<Typography variant="h6" gutterBottom>
								Create Your First Cause
							</Typography>
							<Typography variant="body2" color="text.secondary" mb={2}>
								Causes help you organize your campaigns by theme or area of
								focus.
							</Typography>
							<Button
								variant="contained"
