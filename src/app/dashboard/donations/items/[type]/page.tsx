...
export default function ItemDonationTypePage({ params }: { params: { type: string } }) {
  const router = useRouter();
  const { type } = params;
  const decodedType = decodeURIComponent(type);

  const { data, isLoading, isError } = useGetItemDonationTypeAnalyticsQuery(decodedType);
  ...
  if (isError || !data?.success) {
    return (
      <div className="text-center py-10">
        <p className="text-red-600">Failed to load donation analytics for {decodedType}</p>
        <button
          onClick={() => router.back()}
  ...
  const { stats = { totalDonations: 0, totalQuantity: 0, avgQuantity: 0 },
    recentDonations = [],
    monthlyTrend = [],
    topCauses = [] } = analytics;

  console.log(`Item donation analytics for ${decodedType}:`, { stats, recentDonations, monthlyTrend, topCauses });
      ...
      <div className="mt-8 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            >
              <FaArrowLeft className="mr-2" /> Back to Item Donations
            </Link>
            <div className="flex items-center">
              <div className={`p-3 rounded-full mr-3 ${getDonationTypeColor(decodedType)}`}>
                {getDonationTypeIcon(decodedType)}
              </div>
              <div>
                <h1 className="text-2xl font-bold text-gray-900">{decodedType} Donations</h1>
                <p className="text-gray-600">Detailed analytics for your {decodedType.toLowerCase()} donations</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white rounded-xl shadow-md p-6">
            <div className="flex items-center justify-between">
              ...
              <div className={`p-3 rounded-full ${getDonationTypeColor(decodedType)}`}>
                {getDonationTypeIcon(decodedType)}
              </div>
            </div>
          </div>
        ...
        {stats.totalDonations === 0 && (
          <div className="bg-white rounded-xl shadow-md p-8 text-center mt-6">
            ...
            <h3 className="text-lg font-medium text-gray-900">No {decodedType.toLowerCase()} donations yet</h3>
            <p className="mt-2 text-sm text-gray-600 max-w-md mx-auto">
              You haven't made any {decodedType.toLowerCase()} donations yet. When you donate {decodedType.toLowerCase()}, your donation history and impact will appear here.
            </p>
            <Link
                  ...
                  {monthlyTrend.map((month, index) => {
                    ...
                    return (
                      <div key={index} className="flex flex-col items-center flex-1">
                        <div
                          className={`w-full rounded-t-md ${getDonationTypeColor(decodedType).split(' ')[0].replace('bg-', 'bg-').replace('100', '500')}`}
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        <div className="flex items-center">
                          ...
                          <span className="text-sm text-gray-600">{formatDate(donation.createdAt)}</span>
                        </div>
                        <h3 className="font-medium text-gray-900 mt-1">{donation.description}</h3>
                        <p className="text-sm text-gray-600">
                          {donation.quantity} {donation.unit || 'item'}{donation.quantity !== 1 && 's'}
                        </p>
                        {donation.cause && (
                          <div className="flex items-center mt-1">
                            <FaHandHoldingHeart className="text-gray-400 mr-2" />
                            <span className="text-sm text-gray-600">{donation.cause.title}</span>
                          </div>
                        )}
                      </div>
                      {donation.organization && (
                        <div className="flex items-center">
                          ...
                          <span className="text-sm text-gray-600">{donation.organization.name}</span>
                        </div>
                      )}
                    </div>
                  </div>
            ...
            <div className="space-y-4">
              {topCauses.map((cause, index) => (
                <div key={index} className="border-b border-gray-200 pb-4 last:border-0 last:pb-0">
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="font-medium text-gray-900">{cause.causeName}</h3>
                      <p className="text-sm text-gray-600">
                        {cause.count} donation{cause.count !== 1 && 's'} ({cause.totalQuantity} item{cause.totalQuantity !== 1 && 's'})
                      </p>
                    </div>
                  </div>
                </div>
