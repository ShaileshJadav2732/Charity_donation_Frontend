...
import { ItemDonationAnalytics } from "@/types/itemDonation";
...
export default function ItemDonationsPage() {
  ...
  if (isError || !data?.success) {
    return (
      <div className="text-center py-10">
        <p className="text-red-600">Failed to load item donation analytics</p>
        <button
          onClick={() => router.back()}
  ...
  const analytics: ItemDonationAnalytics = data.data || {
    donationsByType: [],
    monthlyTrend: [],
    topCauses: []
  };
  ...
  console.log("Item donation analytics:", { donationsByType, monthlyTrend, topCauses });
      ...
      <div className="mt-8 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <h1 className="text-2xl font-bold text-gray-900">Item Donation Analytics</h1>
            <p className="text-gray-600">Track your non-monetary donations and their impact</p>
          </div>
        </div>
          ...
          {donationsByType && donationsByType.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {donationsByType.map((type, index) => (
                <Link
                  href={`/dashboard/donations/items/${type.type}`}
                  key={index}
                  className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow"
                >
                  <div className="flex items-center">
                    <div className={`p-3 rounded-full mr-3 ${getDonationTypeColor(type.type)}`}>
                      {getDonationTypeIcon(type.type)}
                    </div>
                    <div>
                      <h3 className="font-medium text-gray-900">{type.type}</h3>
                      <p className="text-sm text-gray-600">
                        {type.count} donation{type.count !== 1 && 's'} ({type.totalQuantity} item{type.totalQuantity !== 1 && 's'})
                      </p>
                    </div>
                  </div>
                </Link>
            ...
            <div className="text-center py-8">
              ...
              <h3 className="text-lg font-medium text-gray-900">No item donations yet</h3>
              <p className="mt-2 text-sm text-gray-600 max-w-md mx-auto">
                You haven't made any item donations yet. When you donate items like clothes, books, or food, they'll appear here.
              </p>
            </div>
          )}
        </div>
        ...
        {topCauses && topCauses.length > 0 && (
          <div className="bg-white rounded-xl shadow-md p-6">
            <h2 className="text-xl font-semibold text-gray-900 mb-4 flex items-center">
              <FaHandHoldingHeart className="mr-2 text-teal-600" /> Top Causes Receiving Item Donations
            </h2>
                ...
                <div key={index} className="border-b border-gray-200 pb-4 last:border-0 last:pb-0">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex space-x-1">
                      {cause.types.map((type, idx) => (
                        <div
                          key={idx}
                          className={`p-2 rounded-full ${getDonationTypeColor(type)}`}
                          title={type}
                        >
